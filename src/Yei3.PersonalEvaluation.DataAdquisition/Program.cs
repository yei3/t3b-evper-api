                ...
                using (var bootstrapper = AbpBootstrapper.Create<PersonalEvaluationDataAdquisitionModule>())
                {
                    ...
                    for (int row = 3; row < rowCount; row++) // start in row 3 cause data starts there, any template change can break this, is better if we provide the template
                    {
                        User currentUser = userRegistrationManager.ImportUser(
                            ...
                            role: worksheet.Cells[row, 11].Value.ToString(),
                            entryDate: worksheet.Cells[row, 12].Value.ToString(),
                            reassignDate: worksheet.Cells[row, 13].Value == null ? null : worksheet.Cells[row, 13].Value.ToString(),
                            birthDate: worksheet.Cells[row, 14].Value.ToString(),
                            scholarship: worksheet.Cells[row, 15].Value.ToString(),
                            email: worksheet.Cells[row, 16].Value.ToString()
                            );
                    }
                }
            }
